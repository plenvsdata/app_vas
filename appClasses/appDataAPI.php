    ...
    public function appCustomerAlarme($data = NULL,$origem,$customerID)
    {
        ...
        if(is_null($data)){
            ...
            if($origem != "OBCON") {
                ...
                $v_nor = ($v_ori == 'O') ? '' : substr($v_alarme,7,50);
                $v_cod = ($v_ori == 'O') ? substr($v_alarme,7,10) : substr($v_alarme,57,10);
                $v_dat = ($v_ori == 'O') ? trim(substr($v_alarme,17,20)) : trim(substr($v_alarme,67,20));
                $v_nuc = ($v_ori == 'O') ? substr($v_alarme,37,3) : substr($v_alarme,87,3);
                $v_apl = ($v_ori == 'O') ? substr($v_alarme,40,10) : substr($v_alarme,90,10);
                $v_ins = ($v_ori == 'O') ? substr($v_alarme,50,2) : substr($v_alarme,100,2);
                $v_org = ($v_ori == 'O') ? substr($v_alarme,52,5) : substr($v_alarme,102,5);
                $v_sub = ($v_ori == 'O') ? substr($v_alarme,57,10) : substr($v_alarme,107,10);
                $v_nsb = ($v_ori == 'O') ? substr($v_alarme,67,5) : substr($v_alarme,117,5);
                $v_sbn = ($v_ori == 'O') ? substr($v_alarme,72,10) : substr($v_alarme,122,10);
                $v_cor = ($v_ori == 'O') ? substr($v_alarme,82,2) : substr($v_alarme,132,2);
                $v_ips = 'NULL';
                $v_pos = 'NULL';
